...
export function execManageApiCapability(args = {}) {
  ...
  try {
    ...
    if (action === 'save_doc') {
      ...
      if (provider !== 'kimi' || kind !== 'vision') {
        return toolJson({
          ok: false,
          tool: 'manage_api_capability',
          error: 'managed v1 currently supports provider="kimi", kind="vision"',
        })
      }
      const slot = saveKimiVisionDocs({
    ...
    if (action === 'enable' || action === 'disable') {
      const slot = setApiCapabilitySlotEnabled(args.slot_id || args.slotId || KIMI_VISION_SLOT_ID, action === 'enable')
    ...
    if (action === 'get') {
      const slot = getApiCapabilitySlot(args.slot_id || args.slotId || KIMI_VISION_SLOT_ID, { includeSecrets: false })
